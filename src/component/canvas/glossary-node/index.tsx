...
export const GlossaryNode: React.FC<GlossaryNodeProps> = ({
  ...
  const onMouseLeave = useCallback(
    (event: React.MouseEvent<HTMLDivElement, MouseEvent>) => {
      GlossaryLinks.filter(
        (notionLink) => notionLink.source === idNumber
      ).forEach((notionLink) => {
      ...
      if (state.dragging) {
        setState((prevState) => ({
          ...
          current: {
            x:
              prevState.current.x +
              (state.position.x - state.relativePosition.x),
            y:
              prevState.current.y +
              (state.position.y - state.relativePosition.y),
          },
        }));
      }
    ...
    [
      ...
      state.position.x,
      state.position.y,
      state.relativePosition.x,
      state.relativePosition.y,
    ]
  );
  ...
  return (
    <div
      ...
      style={{
        transform: `translate(${
          -state.current.x - (state.position.x - state.relativePosition.x)
        }px, ${
          -state.current.y - (state.position.y - state.relativePosition.y)
        }px)`,
      }}
      draggable={true}
