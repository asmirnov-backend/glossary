...
const getNodeCharacteristics = (id: string) => {
  ...
  const translate = sourceNode?.style
...
const connectNodes = () => {
  ...
  GlossaryLinks.forEach((notionLink) => {
    ...
    context?.moveTo(
      sourceWidth / 2 + sourceTranslateX + sourceLeft,
      sourceHeight / 2 + sourceTranslateY + sourceTop
    );

    context?.lineTo(
      targetWidth / 2 + targetTranslateX + targetLeft,
      targetHeight / 2 + targetTranslateY + targetTop
    );
    ...
    const notionRelationTranslateX =
      (sourceWidth / 2 +
        sourceTranslateX +
        sourceLeft +
        targetWidth / 2 +
        ...
        targetLeft) /
      2;
    const notionRelationTranslateY =
      (sourceHeight / 2 +
        sourceTranslateY +
        sourceTop +
        targetHeight / 2 +
    ...
    const notionRelation = document.getElementById(
      `relation-${notionLink.source}-${notionLink.target}`
    );

    const notionRelationWidth = notionRelation?.offsetWidth ?? 0;
    const notionRelationHeight = notionRelation?.offsetHeight ?? 0;

    notionRelation?.style.setProperty(
      "transform",
      `translate(${notionRelationTranslateX - notionRelationWidth / 2}px, ${
        notionRelationTranslateY - notionRelationHeight / 2
      }px)`
    );
  });
};
